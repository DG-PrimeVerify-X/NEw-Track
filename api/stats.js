  ...
  try {
    ...
    const [v, c] = await Promise.all([
      ...
      supabase
        ...
        .select("fingerprint,ref,classification,created_at,dwell_seconds")
        .gte("created_at", since)
    ]);
    ...
    const uniqueVisitors =
      new Set(visits.map(x => x.fingerprint)).size;

    const likelyReal =
      clicks.filter(x => x.classification === "likely_real").length;

    const suspicious =
      clicks.filter(x => x.classification === "suspicious").length;

    const ctr = visits.length
      ? Number(((clicks.length / visits.length) * 100).toFixed(2))
      : 0;
